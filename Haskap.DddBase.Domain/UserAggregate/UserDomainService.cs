...
public class UserDomainService : DomainService
{
    ...
    public UserDomainService(IBaseDbContext baseDbContext)
    {
    ...
    {
        var allPermissions = (from user in _baseDbContext.User.Include(x => x.Roles).Where(x => x.Id == userId)
                              from role in _baseDbContext.Role.Where(x => user.Roles.Select(y => y.RoleId).Contains(x.Id)).DefaultIfEmpty()
    ...
    {
        var rolePermissions = (from user in _baseDbContext.User
